...
namespace Nop.Services.NhaXes
{
    public class LimousineBanVeService : ILimousineBanVeService
    {
        ...
        public virtual List<DatVe> GetSoLanDiTrongThang(int KhachHangId, DateTime NgayDi)
        {
            var query = _datveRepository.Table.Where(c => c.KhachHangId == KhachHangId
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO)
                && (c.NgayDi.Year == NgayDi.Year)
                && (c.NgayDi.Month == NgayDi.Month));
            return query.ToList();
        ...
        public virtual ListDoanhThuTongHop GetDoanhThuTongHopTheoNgay(DateTime NgayDi,int NhaXeId)
        {
            ...
            var datvesnb = _datveRepository.Table.Where(c => chuyendiNB.Contains(c.ChuyenDiId.Value) && (c.TrangThaiId != (int)ENTrangThaiDatVe.HUY));
        ...
        public virtual List<XeDiTrongNgay> GetAllChuyenTheoXeTheoNgay(DateTime TuNgay,DateTime DenNgay, string SerchName, int NhaXeId)
        {
            //lay tat ca chuyen di trong ngay
